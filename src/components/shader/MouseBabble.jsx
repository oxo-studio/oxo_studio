    ...
    const material = new THREE.ShaderMaterial({
      ...
      fragmentShader: /* glsl */ `
        ...
        void mainImage(out vec4 fragColor, in vec2 fragCoord) {
          ...
          // Adjust masterheight with mouse effect on y-axis
          // Map mouse y from 0..height to 0..1 range (inverted y)
          float mouseInfluence = 1.0 - (iMouse.y / iResolution.y);
          ...
          // Apply mouse vertical influence to masterheight for subtle magnet effect
          masterheight *= pow(abs(pinch), 0.1) * -0.02 * mix(1.0, 2.0, mouseInfluence);

          for (int i = 0; i < 3; i++) {
            float noiseofs = noise(vec3(uv.x * 35.0, iTime * 15.0, float(i) * 10.0)) * 2.0 - 1.0;
            float offset = 0.5 + noiseofs * 0.1 * pinch;
